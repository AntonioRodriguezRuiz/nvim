...
dap_python.setup(home .. "/.local/share/nvim/mason/packages/debugpy/venv/bin/python", { console = "integratedTerminal" })
...
function activate_env()
    vim.ui.input({ prompt = 'Env path: ' },
        function(input) vim.cmd('TermExec cmd="source ' .. input .. '/bin/activate.fish"') end)
end
...
function run(custom)
    if not custom then
        ...
        vim.ui.input({ prompt = 'File: ' }, function(input) vim.cmd('TermExec cmd="python ' .. input .. '"') end)
    end
end
...
function debug_remote(custom)
    ...
    if not custom then
        ...
        vim.ui.input({ prompt = 'Port: ' }, function(input) config = debugpy.configure('attach', '127.0.0.1', input) end)
        local final = vim.tbl_extend('keep', config, debugpy_default)
        debugpy.run(final)
    end
end
...
local mappings = {
    P = {
        ...
        v = { function() activate_env() end, "Activate env" },
        r = { function() run() end, "Run main" },
        R = { function() run(true) end, "Run file" },
        d = { function() debug_local() end, "Debug file / program" },
        D = { function() debug_remote() end, "Attach (port 5678)" },
        a = { function() debug_remote(true) end, "Attach (Specify port)" },
        t = {
            name = "Testing",
